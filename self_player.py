...
import pickle
from myagent import Vectorizer
...
class tree():
    ...
    def save_tree_with_count(self,rnds):

        node = self.ultimate_root
        path = "mcts_tree_" + str(rnds) + ".json "
        f = open(path, 'wb')
        p = pickle.Pickler(f)
        p.dump(node)
...
class MCTS_Agent(Agent):
    ...
    def __init__(self, input_big_tree):

        self.type = 'MCTS'

        self.big_tree = input_big_tree
        self.set_root()
    ...
    def set_root(self):
        ...
        if new_game_flag == 0:
            print("New Game")
            self.root_node = self.big_tree.ultimate_root
            self.moves = 0
            ...
            moves = z['move_count']
            self.moves = moves"""
            self.track_moves(move_num=2, opp_move=self.get_opp_move())
            f = open('track_move.json', 'r')
            move_list = json.load(f)
            self.moves = len(move_list)
            node = self.big_tree.ultimate_root
            for x in move_list:
    ...
    def select_uct_child(self, node):
        ...
        S_p = sum(child.num_of_rollouts for child in node.children) + 1  # Total Rollouts or parent rollout
...
if __name__ == "__main__":
    ...
    big_tree_root = tree()
    player = MCTS_Agent()
    #big_tree_root = tree(player.root_node)
    #player.get_input()
    player.gen_output()
    #print(player.get_opp_move())
    big_tree_root.save_tree_in_json()
    print("Time for move = ", time.time() - start)
